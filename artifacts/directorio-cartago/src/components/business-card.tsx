...
export function BusinessCard({
  ...
  const [imageSrc, setImageSrc] = React.useState(() => getBusinessImageSrc(business));

  React.useEffect(() => {
    setImageSrc(getBusinessImageSrc(business));
  }, [business]);
      ...
      <div className="bg-card rounded-2xl overflow-hidden border border-border/50 shadow-sm hover:shadow-xl hover:shadow-primary/10 hover:border-primary/20 transition-all duration-300 h-full flex flex-col relative">
        ...
        <div className="aspect-[4/3] relative overflow-hidden bg-muted">
          <img
            ...
            onError={() => setImageSrc(getBusinessFallbackImage(business.categoryName))}
          />
        ...
        <div className="p-5 flex flex-col flex-1">
          ...
          <div className="mt-auto pt-4 flex items-center justify-between border-t border-border/50">
            <Badge
              variant="secondary"
              className="bg-secondary/10 text-secondary-foreground hover:bg-secondary/20 font-medium"
            >
